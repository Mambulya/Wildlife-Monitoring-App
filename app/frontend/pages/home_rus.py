...
def detect_images(uploaded_files):
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            st.sidebar.download_button(label=":material/download: Загрузить zip-архив", 
                                data=response.content, 
                                file_name="detection.zip", 
                                mime="application/zip")
            end_time = time.time()
            bar.progress(value=100, text=f"Обработано за {end_time - start_time:.2f} seconds")
            status_text.text(f"Обработано за {end_time - start_time:.2f} seconds")
            bar.empty()
